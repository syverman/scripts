...
generate_install_script() {
    ...
    # CORRECCIÓN: Usar pacman -Qqe en lugar de pacman -Qq
...
# -: lee los nombres de los paquetes de la entrada estándar (usando <<< para leer de variable)
...
if [ -n "$PACMAN_PACKAGES_LIST" ]; then # Verifica si la variable no está vacía
    ...
    # Usar sudo para pacman, leer la lista desde la variable usando '<<<'.
    # Esto es equivalente a pasar la lista por stdin pero desde una variable.
...
show_help() {
    ...
    echo "Ejemplo:"
    echo "  ./pc_origen.sh"
    echo "  # Opcional: ./pc_origen.sh create"
